...
function assign_archive_movies_template()
{
    $movies_plugin_options = get_option('movies_plugin_options');

    $page_id = ($movies_plugin_options['archive_movies'] != '') ? $movies_plugin_options['archive_movies'] : '';

    if (get_the_ID() == $page_id) {
        include(MOVIES_PLUGIN_DIR . '/templates/archive-movies.php');
        exit;
    }
}
add_action('template_redirect', 'assign_archive_movies_template');
...
function assign_archive_actor_template()
{
    $movies_plugin_options = get_option('movies_plugin_options');

    $page_id = ($movies_plugin_options['archive_actors'] != '') ? $movies_plugin_options['archive_actors'] : '';

    if (get_the_ID() == $page_id) {
        include(MOVIES_PLUGIN_DIR . '/templates/archive-actor.php');
        exit;
    }
}
add_action('template_redirect', 'assign_archive_actor_template');
