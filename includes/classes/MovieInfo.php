...
class MovieInfo
{
    ...
    /**
     * Retrives the popular movies from the MovieDB API.
     * @method get_popular_movies
     * @return {array} An array of popular movies.
     */
    public function get_popular_movies()
    {
        $movies = array();
        for ($i = 1; $i <= 10; $i++) {
            $endpoint = 'https://api.themoviedb.org/3/movie/popular?api_key=' . $this->api_key . '&language=en-US&page=' . $i;
            ...
            if (!empty($data['results'])) {
                foreach ($data['results'] as $result) {
                    $movie = array(
                      'id' => $result['id'],
                      'title' => $result['title'],
                      'genre_ids' => $this->get_movie_details($result['id'])['genres'],
                      'release_date' => $result['release_date'],
                      'backdrop_path' => isset($result['backdrop_path']) ? 'https://image.tmdb.org/t/p/w500' . $result['backdrop_path'] : 'https://via.placeholder.com/300x300',
                    );
    ...
    */
    public function get_movie_details($movie_id)
    {
        ...
        $movie_details['poster_path']    = isset($data['poster_path']) ? 'https://image.tmdb.org/t/p/w500' . $data['poster_path'] : 'https://via.placeholder.com/500x750';
